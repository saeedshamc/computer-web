...
یک وبسایت فروشگاهی برای فروش قطعات کامپیوتر با امکان بازدید عمومی، سیستم احراز هویت و مدیریت محصولات.

## ویژگی‌ها

- 🏠 **صفحه اصلی عمومی** — مشاهده محصولات بدون نیاز به ورود
- 🔐 ورود و ثبت‌نام فقط برای سفارش / سبد خرید / پروفایل
...
```
کاربر لاگین‌شده → صفحه فروشگاه / چک‌اوت → /api/cart → cartStore → carts.json
                                              ↓
                                         localStorage (کش موقت)
```

1. **ورود:** کاربر با JWT وارد می‌شود.
2. **بارگذاری:** صفحه فروشگاه و چک‌اوت سبد را از `GET /api/cart` می‌گیرند.
...
### داشبورد / صفحه اصلی (`/` و `/dashboard`)
- نمایش عمومی محصولات بدون نیاز به ورود
- جستجو و فیلتر محصولات
- افزودن به سبد و تکمیل خرید فقط پس از ورود
- هدر با منوی ناوبری (ورود / ثبت‌نام برای مهمان)
...
1. کاربر بدون ورود صفحه اصلی را می‌بیند و محصولات را مرور می‌کند.
2. با زدن «افزودن به سبد» یا «سبد خرید / تکمیل خرید» به صفحه ورود هدایت می‌شود.
3. پس از ورود موفق، به مسیر درخواستی (یا صفحه اصلی) بازمی‌گردد و می‌تواند سفارش را ادامه دهد.
